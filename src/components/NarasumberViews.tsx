  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="bg-white border border-[var(--color-line)] rounded-2xl p-6 shadow-sm">
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap gap-3 mb-6">
        <input value={paName} onChange={e=>setPaName(e.target.value)} placeholder="Nama peserta" className="flex-1 min-w-[130px] px-4 py-2 border border-[var(--color-line)] rounded-lg text-sm bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
        <select value={paGroup} onChange={e=>setPaGroup(e.target.value)} className="px-4 py-2 border border-[var(--color-line)] rounded-lg bg-slate-50 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
        ...
        return (
          <motion.div variants={itemVariants} key={g} className="my-4">
            ...
            {mem.map(([c, p]: [string, any]) => (
              <div key={c} className="flex items-center gap-3 py-2 border-b border-[var(--color-line)] flex-wrap last:border-b-0 hover:bg-slate-50 transition-colors">
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap gap-3 mb-4">
        <input value={newGrpName} onChange={e=>setNewGrpName(e.target.value)} placeholder="Nama kelompok baru…" className="flex-1 min-w-[140px] px-4 py-2 border border-[var(--color-line)] rounded-lg text-sm bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
      ...
      <motion.div variants={containerVariants} className="bg-white border border-[var(--color-line)] rounded-xl overflow-hidden">
        {state.config.groups.map(g => (
          <motion.div variants={itemVariants} key={g} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-line)] flex-wrap last:border-b-0 hover:bg-slate-50 transition-colors">
      ...
      <motion.div variants={itemVariants} className="bg-white border border-[var(--color-line)] rounded-2xl p-6 shadow-sm">
        ...
        <div className="border border-[var(--color-line)] rounded-xl overflow-hidden bg-white">
          {rows.map((r, i) => {
            ...
            return (
              <motion.div variants={itemVariants} key={r.name} className="flex items-center gap-4 py-3 px-4 border-b border-[var(--color-line)] last:border-b-0 hover:bg-slate-50 transition-colors">
...
export const NarasumberSkor: React.FC = () => {
  const { state, updateState } = useAppContext();

  const awardXP = async (g: string, amt: number, label: string) => {
    await updateState(draft => {
      draft.groups[g].xp += amt;
      if (!draft.groups[g].xpHistory) draft.groups[g].xpHistory = [];
      draft.groups[g].xpHistory!.unshift({ reason: label, amount: amt, timestamp: Date.now() });
    });
    toast(`${g} +${amt} XP`);
  };

  const setXP = async (g: string, val: number) => {
    await updateState(draft => {
      const oldXp = draft.groups[g].xp;
      const newXp = Math.max(0, val);
      draft.groups[g].xp = newXp;
      if (oldXp !== newXp) {
        if (!draft.groups[g].xpHistory) draft.groups[g].xpHistory = [];
        draft.groups[g].xpHistory!.unshift({ reason: "Edit Manual", amount: newXp - oldXp, timestamp: Date.now() });
      }
    });
    toast("XP diperbarui");
  };

  const CustomXP: React.FC<{ g: string }> = ({ g }) => {
    const [amt, setAmt] = useState(10);
    return (
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-200">
        <input type="number" value={amt} onChange={e => setAmt(parseInt(e.target.value) || 0)} className="w-20 text-center py-1.5 border border-[var(--color-line)] rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 font-semibold text-sm" />
        <button onClick={() => awardXP(g, amt, "Custom")} className="px-3 py-1.5 bg-indigo-600 text-white rounded-md font-semibold text-sm hover:bg-indigo-700 shadow-sm transition-colors">Tambah XP</button>
      </div>
    );
  };
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="bg-white border border-[var(--color-line)] rounded-2xl p-6 shadow-sm">
      <motion.div variants={itemVariants} className="text-[10px] tracking-widest uppercase text-[var(--color-muted)] font-bold mb-1">Fasilitasi · khusus narasumber</motion.div>
      <motion.h2 variants={itemVariants} className="text-xl font-bold text-[var(--color-ink)] mb-2 flex items-center gap-2">⚡ Beri XP kelompok</motion.h2>
      <motion.p variants={itemVariants} className="text-[var(--color-muted)] text-sm mb-6">Hanya narasumber yang dapat memberi XP. XP kolektif per kelompok.</motion.p>
      
      <motion.div variants={containerVariants}>
        {state.config.groups.map(g => {
          const d = state.groups[g];
          const li = levelOf(d.xp);
          return (
            <motion.div variants={itemVariants} key={g} className="border border-[var(--color-line)] rounded-xl p-5 mb-4 bg-slate-50">
              <div className="flex justify-between items-center flex-wrap gap-2 mb-4">
                <span className="font-bold text-lg text-[var(--color-ink)]">{g}</span>
                <span className="inline-block px-3 py-1 rounded-md text-xs font-bold bg-amber-50 text-amber-700 uppercase tracking-wide">{LEVELS[li].name} · {d.xp} XP</span>
              </div>
              <div className="flex flex-wrap gap-2 mb-4">
                {Object.entries(XP).map(([k, v]) => (
                  <button key={k} onClick={() => awardXP(g, v, k)} className="px-4 py-2 border border-[var(--color-line)] rounded-md bg-white text-xs font-semibold text-[var(--color-ink)] hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-200 transition-colors shadow-sm">{k} +{v}</button>
                ))}
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-[var(--color-line)]">
                <span className="text-sm font-semibold text-[var(--color-ink)] mr-2">Edit Total:</span>
                <button onClick={() => setXP(g, d.xp - 10)} className="w-8 h-8 rounded-md border border-[var(--color-line)] bg-white text-lg font-bold text-red-600 hover:bg-red-50 transition-colors flex items-center justify-center">−</button>
                <input type="number" value={d.xp} onChange={e => setXP(g, parseInt(e.target.value)||0)} className="w-16 text-center py-1.5 border border-[var(--color-line)] rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 font-semibold" />
                <button onClick={() => setXP(g, d.xp + 10)} className="w-8 h-8 rounded-md border border-[var(--color-line)] bg-white text-lg font-bold text-indigo-600 hover:bg-indigo-50 transition-colors flex items-center justify-center">+</button>
                <button onClick={() => setXP(g, 0)} className="px-4 py-1.5 ml-2 bg-white text-red-600 border border-red-200 rounded-md font-semibold text-xs hover:bg-red-50 transition-colors shadow-sm">Reset XP</button>
              </div>
              <CustomXP g={g} />
            </motion.div>
          );
...
export const NarasumberNilai: React.FC = () => {
  const { state, updateState } = useAppContext();
  ...
  const GroupRow: React.FC<{ g: string }> = ({ g }) => {
    ...
    const save = async () => {
      await updateState(draft => { draft.groups[g].scores = scores; });
      toast("Nilai " + g + " tersimpan ✔");
    };

    const tot = studTotal(scores);

    const awardTotalAsXP = async () => {
      if (tot > 0) {
        await updateState(draft => {
          draft.groups[g].xp += tot;
          if (!draft.groups[g].xpHistory) draft.groups[g].xpHistory = [];
          draft.groups[g].xpHistory!.unshift({ reason: "Nilai Proyek", amount: tot, timestamp: Date.now() });
        });
        toast(`+${tot} XP untuk ${g} ✔`);
      }
    };

    const awardXP = async (amt: number) => {
      await updateState(draft => {
        draft.groups[g].xp += amt;
        if (!draft.groups[g].xpHistory) draft.groups[g].xpHistory = [];
        draft.groups[g].xpHistory!.unshift({ reason: "Tambahan Manual", amount: amt, timestamp: Date.now() });
      });
    ...
    return (
      <div className="border border-[var(--color-line)] rounded-xl p-4 mb-3 bg-slate-50">
        <div className="flex justify-between items-center gap-2 flex-wrap mb-3">
          ...
          <span className="font-serif text-2xl text-indigo-700 font-bold">{tot}</span>
        </div>
        {RUBRIC.map(k => {
          const val = scores[k] || 0;
          return (
            <div key={k} className="flex items-center gap-2 flex-wrap my-1.5">
              <span className="text-sm font-medium text-[var(--color-muted)] min-w-[120px] flex-1">{k}</span>
              <div className="flex items-center gap-1.5">
                ...
                <input type="number" min="0" max="100" value={val} onChange={e => setScore(k, parseInt(e.target.value)||0)} className="w-14 text-center py-1 border border-[var(--color-line)] rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 font-semibold" />
        ...
        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-200">
          <button onClick={save} className="px-4 py-2 bg-indigo-600 text-white rounded-md font-semibold text-sm hover:bg-indigo-700 shadow-sm transition-colors flex-1 sm:flex-none">💾 Simpan Nilai</button>
          <button onClick={awardTotalAsXP} className="px-4 py-2 bg-amber-500 text-white rounded-md font-semibold text-sm hover:bg-amber-600 shadow-sm transition-colors flex-1 sm:flex-none">⚡ Beri +{tot} XP (Total)</button>
        </div>
        
        <div className="mt-3 pt-3 border-t border-slate-200">
          <span className="text-xs font-semibold text-[var(--color-muted)] block mb-2 uppercase tracking-wider">Tambah XP Manual</span>
          <div className="flex flex-wrap gap-2">
            {[15, 20, 30, 50].map(amt => (
              <button key={amt} onClick={() => awardXP(amt)} className="px-3 py-1.5 bg-emerald-100 text-emerald-800 rounded-md font-bold text-xs hover:bg-emerald-200 transition-colors">
                +{amt} XP
              </button>
            ))}
          </div>
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="bg-white border border-[var(--color-line)] rounded-2xl p-6 shadow-sm">
      <motion.div variants={itemVariants} className="text-[10px] tracking-widest uppercase text-[var(--color-muted)] font-bold mb-1">Penilaian Kelompok · Khusus Narasumber</motion.div>
      <motion.h2 variants={itemVariants} className="text-xl font-bold text-[var(--color-ink)] mb-2 flex items-center gap-2">📝 Nilai Kelompok</motion.h2>
      <motion.p variants={itemVariants} className="text-[var(--color-muted)] text-sm mb-6">Nilai proyek kelompok. Bobot 20% untuk tiap kriteria.</motion.p>
              ...
              {has ? (
                <div className="space-y-4">
                  {TASKS.map(task => {
                    ...
                    return (
                      <div key={task.id} className="bg-white border border-[var(--color-line)] rounded-lg p-4 shadow-sm">
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="bg-white border border-[var(--color-line)] rounded-2xl p-6 shadow-sm">
      ...
      <motion.div variants={containerVariants}>
        {MATERIALS.map(m => {
          ...
          return (
            <motion.div variants={itemVariants} key={m.id} className="border border-[var(--color-line)] rounded-xl p-5 mb-4 bg-slate-50 flex flex-col md:flex-row md:items-center gap-6">
